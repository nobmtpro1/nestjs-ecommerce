...
export class Migration1699323577422 implements MigrationInterface {
    name = 'Migration1699323577422'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP FOREIGN KEY \`FK_3a209bfb9e1491419d8ef9e01bb\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP FOREIGN KEY \`FK_5a3dca5d60a9fb5aca8f7bf0cdd\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP FOREIGN KEY \`FK_cc14797500bad5e867eb4f5e1e2\``);
        await queryRunner.query(`CREATE TABLE \`product_option\` (\`id\` varchar(36) NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`position\` int NOT NULL, \`values\` text NOT NULL, \`productId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`productAttributeValue1Id\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`productAttributeValue2Id\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`productAttributeValue3Id\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`option1\` varchar(255) NULL`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`option2\` varchar(255) NULL`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`option3\` varchar(255) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`product\` CHANGE \`type\` \`type\` varchar(255) NOT NULL DEFAULT 'VARIABLE'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`product\` CHANGE \`type\` \`type\` varchar(255) NOT NULL DEFAULT ''simpleData''`);
        await queryRunner.query(`ALTER TABLE \`product\` ADD CONSTRAINT \`FK_b1b332c0f436897f21a960f26c7\` FOREIGN KEY (\`imageId\`) REFERENCES \`image\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`product_category\` ADD CONSTRAINT \`FK_569b30aa4b0a1ad42bcd30916aa\` FOREIGN KEY (\`parentId\`) REFERENCES \`product_category\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_category\` ADD CONSTRAINT \`FK_f38e86bd280ff9c9c7d9cb78393\` FOREIGN KEY (\`imageId\`) REFERENCES \`image\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`option3\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`option2\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` DROP COLUMN \`option1\``);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`productAttributeValue3Id\` varchar(36) NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`productAttributeValue2Id\` varchar(36) NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD \`productAttributeValue1Id\` varchar(36) NULL DEFAULT 'NULL'`);
        ...
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD CONSTRAINT \`FK_cc14797500bad5e867eb4f5e1e2\` FOREIGN KEY (\`productAttributeValue3Id\`) REFERENCES \`product_attribute_value\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD CONSTRAINT \`FK_5a3dca5d60a9fb5aca8f7bf0cdd\` FOREIGN KEY (\`productAttributeValue2Id\`) REFERENCES \`product_attribute_value\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_variant\` ADD CONSTRAINT \`FK_3a209bfb9e1491419d8ef9e01bb\` FOREIGN KEY (\`productAttributeValue1Id\`) REFERENCES \`product_attribute_value\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
