...
export class UpdateProductSimpleData1698026670456 implements MigrationInterface {
    name = 'UpdateProductSimpleData1698026670456'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` ADD \`weight\` float NULL`);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` ADD \`length\` float NULL`);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` ADD \`width\` float NULL`);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` ADD \`height\` float NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`product_category\` ADD CONSTRAINT \`FK_569b30aa4b0a1ad42bcd30916aa\` FOREIGN KEY (\`parentId\`) REFERENCES \`product_category\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_category\` ADD CONSTRAINT \`FK_f38e86bd280ff9c9c7d9cb78393\` FOREIGN KEY (\`imageId\`) REFERENCES \`image\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` DROP COLUMN \`height\``);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` DROP COLUMN \`width\``);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` DROP COLUMN \`length\``);
        await queryRunner.query(`ALTER TABLE \`product_simple_data\` DROP COLUMN \`weight\``);
    }
